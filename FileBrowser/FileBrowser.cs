...
namespace PaJaMa.WinControls.FileBrowser
{
	...
	public delegate void SelectedFilesChangedEventHandler(object sender, SelectedFilesChangedEventArgs e);
    ...
    /// <summary>
    /// This enum is used to indicate which folder should be opened when initialising the FileBrowser.
    /// When 'Other' is chosen, the FileBrowser will start in the directory given by the StartUpDirectoryOther
    /// property.
    /// </summary>
    ...
    public partial class FileBrowser : UserControl
    {
        ...
        // The shellFileBrowser used by this FileBrowser to get the shellitems of all files and folders
        private ShellFileBrowser shellFileBrowser;

        // These wrappers are used for accepting drops on the FileBrowser
        private FileBrowserTVDropWrapper tvDropWrapper;
        private FileBrowserLVDropWrapper lvDropWrapper;

        // These wrappers are used to allow dragging from the FileBrowser
        private FileBrowserTVDragWrapper tvDragWrapper;
        private FileBrowserLVDragWrapper lvDragWrapper;

        // These wrappers are used to create the standard context menu's, like in Windows Explorer
        private FileBrowserTVContextMenuWrapper tvContextWrapper;
        private FileBrowserLVContextMenuWrapper lvContextWrapper;

        private FileBrowserPluginWrapper pluginWrapper;
        ...
        // These fields are used to determine the directory to start the FileBrowser in
		...
		public event SelectedFilesChangedEventHandler SelectedFilesChanged;
        ...
        #region Init FileBrowser

        #region At Constructor

        /// <summary>
        /// Inits the shellFileBrowser and registeres some events
        /// </summary>
        private void InitFileBrowser()
        {
            ...
            Load += new EventHandler(FileBrowser_Load);
        ...
        private void InitFileView()
        {
			...
			fileView.ItemSelectionChanged += new ListViewItemSelectionChangedEventHandler(fileView_ItemSelectionChanged);
        }
        ...
        private void InitPlugins()
        {
            if (PluginWrapper == null)
                PluginWrapper = new FileBrowserPluginWrapper();
        ...
        {
            tvContextWrapper = new FileBrowserTVContextMenuWrapper(this);
            lvContextWrapper = new FileBrowserLVContextMenuWrapper(this, PluginWrapper);
        }

        /// <summary>
        /// If the FileBrowser allows dropping, the wrappers for drag/drop are initialised and the events are registered
        /// </summary>
        private void InitDragDrop()
        {
            if (AllowDrop)
            {
                tvDropWrapper = new FileBrowserTVDropWrapper(this);
                lvDropWrapper = new FileBrowserLVDropWrapper(this);

                tvDropWrapper.Drop += new DropEventHandler(DropWrapper_Drop);
                lvDropWrapper.Drop += new DropEventHandler(DropWrapper_Drop);

                tvDragWrapper = new FileBrowserTVDragWrapper(this);
                lvDragWrapper = new FileBrowserLVDragWrapper(this);
        ...
        /// <summary>
        /// Selects the startup directory of the FileBrowser
        /// </summary>
        ...
        {
            if (ShellFileBrowser == null)
                ShellFileBrowser = new ShellFileBrowser();

            desktopNode = new TreeNode(
                ShellFileBrowser.DesktopItem.Text,
                ShellFileBrowser.DesktopItem.ImageIndex,
                ShellFileBrowser.DesktopItem.SelectedImageIndex);
            desktopNode.Tag = ShellFileBrowser.DesktopItem;
            desktopNode.Name = desktopNode.Text;

            folderView.Nodes.Add(desktopNode);
            navAddressBox.Items.Clear();
            navAddressBox.Items.Add(new FileBrowserComboItem(ShellFileBrowser.DesktopItem, 0));

            navAddressBox.CurrentItem = (FileBrowserComboItem)navAddressBox.Items[0];
            selectedNode = desktopNode;
            selectedItem = ShellFileBrowser.DesktopItem;

            ShellFileBrowser.DesktopItem.Expand(false, true, IntPtr.Zero);

            foreach (ShellItem desktopChild in ShellFileBrowser.DesktopItem.SubFolders)
            {
                ...
                navAddressBox.Items.Add(new FileBrowserComboItem(desktopChild, 1));

                if (desktopChildNode.Text == ShellFileBrowser.MyComputerName)
                {
                    ...
                    foreach (ShellItem myCompChild in desktopChild.SubFolders)
                    {
                        ...
                        navAddressBox.Items.Add(new FileBrowserComboItem(myCompChild, 2));
        ...
        private void InitUpdate()
        {
            ...
            ShellFileBrowser.ShellItemUpdate += new ShellItemUpdateEventHandler(shellFileBrowser_ShellItemUpdate);
            updateThread.Start();
        }
        ...
        #region FileBrowser Events

        void FileBrowser_Load(object sender, EventArgs e)
        {
        ...
        /// <summary>
        /// When the handle of the FileBrowser is created all wrappers are initialised and the startup directory is selected.
        ...
        void FileBrowser_HandleDestroyed(object sender, EventArgs e)
        {
            if (handleCreated)
            {
                ...
                ShellFileBrowser.UpdateCondition.ContinueUpdate = false;
		...
		void fileView_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
		{
			if (SelectedFilesChanged != null)
				SelectedFilesChanged(sender, new SelectedFilesChangedEventArgs((ShellItem)e.Item.Tag));
		}
        ...
        private void UpdateLoop()
        {
            while (updating)
            {
                ...
                {
                    ShellFileBrowser.UpdateCondition.ContinueUpdate = true;
        ...
        void shellFileBrowser_ShellItemUpdate(object sender, ShellItemUpdateEventArgs e)
        {
            if (updating)
            {
        ...
        private string ConvertPath(string path)
        {
            ...
            if (newPath.StartsWith(
                    string.Format(@"{0}\", ShellFileBrowser.MyComputerName),
                    false,
        ...
        private bool PathExists(string path)
        {
            ...
            {
                bool found = false;
                if (ShellFileBrowser.DesktopItem.SubFolders.Contains(pathParts[i]))
                {
                    pathParts[i] = ShellItem.GetRealPath(
                        ShellFileBrowser.DesktopItem.SubFolders[pathParts[i]]);
                ...
                {
                    ShellItem myComp =
                        ShellFileBrowser.DesktopItem.SubFolders[ShellFileBrowser.MyComputerName];
        ...
        public TreeNode SelectPath(SpecialFolders specialFolder, bool expandNode)
        {
            ...
            {
                path.Replace(ShellFileBrowser.MyDocumentsPath, ShellFileBrowser.MyDocumentsName);
            ...
            else
            {
                ...
                if (specialFolder == SpecialFolders.MyDocuments)
                {
                    ...
                    ShellFileBrowser.DesktopItem.ShellFolder.ParseDisplayName(
                        IntPtr.Zero,
                        IntPtr.Zero,
                ...
                if (pidl != IntPtr.Zero)
                {
                    ...
                    if (ShellFileBrowser.DesktopItem.ShellFolder.GetDisplayNameOf(
                                    pidl,
        ...
        /// <summary>
        /// Selects a path from an existing ShellItem, this ShellItem must be present in the FileBrowsers 
        /// ShellFileBrowser, otherwise it can't be selected.
        /// </summary>
        ...
        public TreeNode SelectPath(ShellItem item, bool expandNode)
        {
            ...
            ShellItem[] path = ShellFileBrowser.GetPath(item);
        ...
        private bool SetNewPath(ShellItem oldItem, ShellItem newItem)
        {
            ...
            if (oldItem != newItem && newItem.Expand(true, false, Handle))
            {
                ...
                {
                    bool used = false;
                    foreach (FileBrowser br in ShellFileBrowser.ShellFileBrowsers)
                    {
        ...
        [Browsable(false)]
        internal FileBrowserTreeView FolderView { get { return folderView; } }

        [Browsable(false)]
        internal FileBrowserListView FileView { get { return fileView; } }

        [Browsable(false)]
        internal FileBrowserComboBox NavAddressBox { get { return navAddressBox; } }
        ...
        [Category("Options"),
         Description("Sets the ShellFileBrowser for the control, if null the control will create it's own."),
         DefaultValue(null),
         Browsable(true)]
        public ShellFileBrowser ShellFileBrowser
        {
            get { return shellFileBrowser; }
            set
            {
                if (!ShellDll.ShellFileBrowser.Equals(ShellFileBrowser, value))
                {
                    if (ShellFileBrowser != null)
						ShellFileBrowser.ShellFileBrowsers.Remove(this);

                    if (handleCreated) FileBrowser_HandleDestroyed(this, new EventArgs());
                    shellFileBrowser = value;
                    if (handleCreated) FileBrowser_HandleCreated(this, new EventArgs());

					if (!ShellFileBrowser.ShellFileBrowsers.Contains(this))
						ShellFileBrowser.ShellFileBrowsers.Add(this);
                }
            }
        }

        [Category("Options"),
         Description("Sets the FileBrowserPluginWrapper for the control, if null the control will create it's own."),
         DefaultValue(null),
         Browsable(true)]
        public FileBrowserPluginWrapper PluginWrapper
        {
            get { return pluginWrapper; }
            set
            {
                if (!FileBrowserPluginWrapper.Equals(PluginWrapper, value))
                {
                    if (handleCreated) FileBrowser_HandleDestroyed(this, new EventArgs());
                    pluginWrapper = value;
                    if (handleCreated) FileBrowser_HandleCreated(this, new EventArgs());
        ...
        public bool FileBrowserForward()
        {
		...
		public ShellItem[] GetSelectedFiles()
		{
			if (fileView.SelectedItems != null)
			{
				List<ShellItem> items = new List<ShellItem>();
				foreach (ListViewItem node in fileView.SelectedItems)
				{
					items.Add((ShellItem)node.Tag);
				}
		...
		public ShellItem GetSelectedFolder()
		{
			if (folderView.SelectedNode != null)
				return (ShellItem)folderView.SelectedNode.Tag;
			return null;
		}
        ...
        /// <summary>
        /// When an item is being dragged from the FileBrowser this method will set the dragStartControl to the
        ...
        /// <summary>
        /// When an item is being dropped on the FileBrowser while holding the right mouse button, the context
        ...
        /// directory of the FileBrowser. It will add and remove items to make it look like the Windows Explorer
        /// ComboBox.
        /// </summary>
        private void ChangeNavBarItem(SelectedFolderChangedEventArgs e)
        {
            if (!navAddressBox.CurrentItem.ShellItem.Equals(e.Item))
            {
                ...
                FileBrowserComboItem currentItem = navAddressBox.CurrentItem;
                ...
                {
                    navAddressBox.Items.Insert(currentIndex + 1, new FileBrowserComboItem(e.Item, e.Node.Level));
                ...
                else if (folderView.IsParentNode(e.Node, selectedNode, out path))
                {
                    ...
                    while (currentItem.Text != e.Node.Text && currentItem.Indent >= maxIndent)
                    {
                        ...
                        currentIndex--;
                        currentItem = (FileBrowserComboItem)navAddressBox.Items[currentIndex];
                    }
                }
                else
                {
                        ...
                        if (path.Length > 2)
                        {
                            ...
                            for (int i = 2; i < path.Length; i++)
                            {
                                navAddressBox.Items.Insert(startIndex + i,
                                    new FileBrowserComboItem((ShellItem)path[i].Tag, i + 1));
                            }
                    ...
                    else if (folderView.IsParentNode(desktopNode, e.Node, out path))
                    {
                        if (path.Length > 2)
                        {
                            ...
                            {
                                navAddressBox.Items.Insert(startIndex + i,
                                    new FileBrowserComboItem((ShellItem)path[i].Tag, i));
                            }
        ...
        void navAddressBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                ShellItem item = ((FileBrowserComboItem)navAddressBox.Items[navAddressBox.SelectedIndex]).ShellItem;
                ...
                if (!selectedItem.Equals(item))
                {
                    ...
                    navAddressBox.CurrentItem = (FileBrowserComboItem)navAddressBox.Items[navAddressBox.SelectedIndex];
	...
	public class SelectedFilesChangedEventArgs : EventArgs
	{
		private ShellItem _item;

		public SelectedFilesChangedEventArgs(ShellItem item)
		{
			_item = item;
		}

		public ShellItem Item { get { return _item; } }
	}
