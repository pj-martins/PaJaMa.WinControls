...
namespace ShellDll
{
    public sealed class ShellItem : IEnumerable, IDisposable, IComparable
    {
        ...
        private ShellFileBrowser fileBrowser;
        ...
        internal ShellItem(ShellFileBrowser fileBrowser, IntPtr pidl, IntPtr shellFolderPtr)
        {
        ...
        internal ShellItem(ShellFileBrowser fileBrowser, ShellItem parentItem, IntPtr pidl, IntPtr shellFolderPtr)
        {
            this.fileBrowser = fileBrowser;
        ...
        internal ShellItem(ShellFileBrowser fileBrowser, ShellItem parentItem, IntPtr pidl)
        {
            this.fileBrowser = fileBrowser;
        ...
        internal bool Expand(bool expandFiles, bool expandFolders, IntPtr winHandle)
        {
                ...
                (expandFiles || expandFolders) && ShellFolder != null && !disposed)
            {
                ...
                try
                {
                    ...
                    if (expandFiles)
                    {
                        if (this.Equals(FileBrowser.DesktopItem) || parentItem.Equals(FileBrowser.DesktopItem))
                        {
                                    ...
                                    out fileEnumPtr) == ShellAPI.S_OK)
                            {
                                ...
                                while (fileEnum.Next(1, out pidlSubItem, out celtFetched) == ShellAPI.S_OK && celtFetched == 1)
                                {
                                    ...
                                    {
                                        ShellItem newItem = new ShellItem(FileBrowser, this, pidlSubItem);
        ...
        {
            if (FileBrowser.UpdateCondition.ContinueUpdate && (updateFiles || updateFolders) && ShellFolder != null && !disposed)
            {
                lock (FileBrowser)
                {
                    #region Fields
                    ...
                    try
                    {
                        ...
                        if (FileBrowser.UpdateCondition.ContinueUpdate && updateFiles)
                        {
                            ...
                            if (this.Equals(FileBrowser.DesktopItem) || parentItem.Equals(FileBrowser.DesktopItem))
                            {
                                        ...
                                        out fileEnumPtr) == ShellAPI.S_OK)
                                {
                                    fileEnum = (IEnumIDList)Marshal.GetTypedObjectForIUnknown(fileEnumPtr, typeof(IEnumIDList));
                                    ShellAPI.SFGAO attribs = ShellAPI.SFGAO.FOLDER;
                                    while (FileBrowser.UpdateCondition.ContinueUpdate &&
                            ...
                            else
                            {
                                        ...
                                        out fileEnumPtr) == ShellAPI.S_OK)
                                {
                                           ...
                                           fileEnum.Next(1, out pidlSubItem, out celtFetched) == ShellAPI.S_OK && celtFetched == 1)
                                    {
                                        if ((index = subFiles.IndexOf(pidlSubItem)) == -1)
                                        {
                                            add.Add(new ShellItem(FileBrowser, this, pidlSubItem));
                                        }
                                        else if (index < fileExists.Length)
                                        {
                                            fileExists[index] = true;
                            ...
                            for (int i = 0; fileEnumCompleted && FileBrowser.UpdateCondition.ContinueUpdate && i < fileExists.Length; i++)
                            {
                            ...
                            if (fileEnumCompleted && FileBrowser.UpdateCondition.ContinueUpdate)
                            {
                                int newIndex;
                                foreach (ShellItem oldItem in remove)
                                {
                                    if ((newIndex = add.IndexOf(oldItem.Text)) > -1)
                                    {
                                        ...
                                        FileBrowser.OnShellItemUpdate(this, new ShellItemUpdateEventArgs(oldItem, oldItem, ShellItemUpdateType.Updated));
                                    }
                                    else
                                    {
                                        subFiles.Remove(oldItem);
                                        FileBrowser.OnShellItemUpdate(this, new ShellItemUpdateEventArgs(oldItem, null, ShellItemUpdateType.Deleted));
                                ...
                                {
                                    subFiles.Add(newItem);
                                    FileBrowser.OnShellItemUpdate(this, new ShellItemUpdateEventArgs(null, newItem, ShellItemUpdateType.Created));
                                }
                        ...
                        if (FileBrowser.UpdateCondition.ContinueUpdate && updateFolders)
                        {
                            ShellItemCollection add = new ShellItemCollection(this);
                            ShellItemCollection remove = new ShellItemCollection(this);
                            ...
                            for (int i = 0; folderEnumCompleted && FileBrowser.UpdateCondition.ContinueUpdate && i < folderExists.Length; i++)
                            {
                            ...
                            if (folderEnumCompleted && FileBrowser.UpdateCondition.ContinueUpdate)
                            {
                                int newIndex;
                                foreach (ShellItem oldItem in remove)
                                {
                    ...
                    finally
                    {
                        ...
                        if (fileEnum != null)
                        {
                            ...
                            if (!(type == FileBrowser.SystemFolderName && string.Compare(text, "Control Panel", true) == 0))
                                Marshal.Release(fileEnumPtr);
                        }
        ...
        {
            FileBrowser.UpdateCondition.ContinueUpdate = false;
            lock (FileBrowser)
            {
                try
                {
                    ...
                    FileBrowser.OnShellItemUpdate(this, new ShellItemUpdateEventArgs(null, item, ShellItemUpdateType.Created));
                }
                catch (Exception) { }
            }
        }
        ...
        internal void Update(IntPtr newPidlFull, ShellItemUpdateType changeType)
        {
            ...
            FileBrowser.OnShellItemUpdate(ParentItem, new ShellItemUpdateEventArgs(this, this, changeType));
        }
        ...
        internal void RemoveItem(ShellItem item)
        {
            ...
            lock (FileBrowser)
            {
                try
                {
                    ...
                    FileBrowser.OnShellItemUpdate(this, new ShellItemUpdateEventArgs(item, null, ShellItemUpdateType.Deleted));
        ...
        {
            if (item.Equals(item.FileBrowser.DesktopItem))
            {
                return "::{450d8fba-ad25-11d0-98a8-0800361b1103}";
            }
            else if (item.Type == item.FileBrowser.SystemFolderName)
            {
                IntPtr strr = Marshal.AllocCoTaskMem(ShellAPI.MAX_PATH * 2 + 4);
                Marshal.WriteInt32(strr, 0, 0);
                StringBuilder buf = new StringBuilder(ShellAPI.MAX_PATH);
        ...
        internal ShellFileBrowser FileBrowser { get { return fileBrowser; } }
        ...
        public bool IsSystemFolder { get { return type == FileBrowser.SystemFolderName; } }
        ...
        private static short MakeSortFlag(ShellItem item)
        {
            if (item.IsFolder)
            {
                if (item.IsDisk)
                    return 1;
                if (item.Text == item.FileBrowser.MyDocumentsName &&
                    item.Type == item.FileBrowser.SystemFolderName)
                    return 2;
                else if (item.Text == item.FileBrowser.MyComputerName)
                    return 3;
                else if (item.Type == item.FileBrowser.SystemFolderName)
                {
                    if (!item.IsBrowsable)
